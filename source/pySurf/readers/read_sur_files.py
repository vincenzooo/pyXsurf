...
def readsur(filepath,raw=False):
    ...
    res.format              =   fread(fileID,1,np.int16);            #2
    res.objNum              =   fread(fileID,1,np.int16);            #3
    res.version             =   fread(fileID,1,np.int16);            #4
    res.objType             =   fread(fileID,1,np.int16);            #5
    res.objName             =   freadChar(fileID,30);               #6
    res.operatorName        =   freadChar(fileID,30);               #7

    res.materialCode        =   fread(fileID,1,np.int16);            #8
    res.acquisitionType     =   fread(fileID,1,np.int16);            #9
    res.rangeType           =   fread(fileID,1,np.int16);            #10
    res.specialPoints       =   fread(fileID,1,np.int16);            #11
    res.absoluteHeights     =   fread(fileID,1,np.int16);            #12
    res.gaugeResolution     =   fread(fileID,1,np.single); #fread(fileID,1,'float');            #13

    freadChar(fileID,4);        #reserved space. for unknown reason this works in matlab, not in python  #14

    #data format of pints
    res.sizeOfPoints        =   fread(fileID,1,np.int16);            #15

    res.zMin                =   fread(fileID,1,np.int32);            #16
    res.zMax                =   fread(fileID,1,np.int32);            #17

    #Number of points per axis
    res.xPoints             =   int(fread(fileID,1,np.int32))            #18
    res.yPoints             =   int(fread(fileID,1,np.int32))            #19

    res.totalNumberOfPoints =   fread(fileID,1,np.int32);            #20
    
    #Distance between points
    res.xSpacing            =   fread(fileID,1,np.single) #'float'); #21
    res.ySpacing            =   fread(fileID,1,np.single) #'float'); #22
    res.zSpacing            =   fread(fileID,1,np.single) #'float'); #23
    ...
    res.xUnitRatio          =   fread(fileID,1,np.single) #'float'); #33
    res.yUnitRatio          =   fread(fileID,1,np.single) #'float'); #34
    res.zUnitRatio          =   fread(fileID,1,np.single) #'float'); #35

    res.imprint             =   fread(fileID,1,np.int16) #'float'); #36
    res.inverted            =   fread(fileID,1,np.int16) #'float'); #37
    res.levelled            =   fread(fileID,1,np.int16) #'float'); #38
    freadChar(fileID,12);       #obsolete                           #39

    #timestamp
    res.startSeconds        =   fread(fileID,1,np.int16);            #40
    res.startMinutes        =   fread(fileID,1,np.int16);            #41
    res.startHours          =   fread(fileID,1,np.int16);            #42
    res.startDays           =   fread(fileID,1,np.int16);            #43
    res.startMonths         =   fread(fileID,1,np.int16);            #44
    res.startYears          =   fread(fileID,1,np.int16);            #45
    res.startWeekDay        =   fread(fileID,1,np.int16);            #46
    res.measurementDuration =   fread(fileID,1,np.single) #'float'); #47

    freadChar(fileID,10);       #obsolete                           #48

    #Size of comment field
    res.commentSize         =   fread(fileID,1,np.int16);            #49
    res.privateSize         =   fread(fileID,1,np.int16);            #50

    res.clientZone          =   freadChar(fileID,128);              #51

    #Axis offset
    res.xOffset             =   fread(fileID,1,np.single) #'float'); #52
    res.yOffset             =   fread(fileID,1,np.single) #'float'); #53
    res.zOffset             =   fread(fileID,1,np.single) #'float'); #54

    #temperature scale
    res.tSpacing            =   fread(fileID,1,np.single) #'float'); #55
    res.tOffset             =   fread(fileID,1,np.single) #'float'); #56
    ...
    if res.sizeOfPoints == 16:                                       #61
        res.points          =   fread(fileID,-1,np.int16)
    elif res.sizeOfPoints == 32:
        res.points          =   fread(fileID,-1,np.int32)        
    else:
