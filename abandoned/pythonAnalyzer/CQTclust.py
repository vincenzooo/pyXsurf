...
import numpy as np
#from pylab import *
from abandoned.pythonAnalyzer.distances import *
import pdb
...
def QTclust(punti,Dmax,distance=defaultDist):
    ...
    print("Distance Table of %i punti, max:%8.3f"%(len(p),np.nanmax(disTab)))
    clusterList=[]
    max_ccm=0
    minrad=0
    #pdb.set_trace()
    for j in range(npt):
        ...
        ccrad=0
        col=disTab[j]
        interestArg= np.nonzero(col<Dmax)[0]  #np.nonzero(less(col,Dmax))   #array di indici di valori che possono essere presi in considerazione
        ...
        interestVal= col[col<Dmax] #np.compress(np.less(col,Dmax),col)[0]  #array di valori che possono essere presi in considerazione
        sv=np.argsort(interestVal) #indici ordinati dal piu' piccolo al + grande
