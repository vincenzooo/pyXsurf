"""
2018/06/06 v 1.2
Attempt to modify interface in a way that modify self and return it. It is inconvenient to have always to create a copy. Revert in 1.3
...
from pySurf.data2D import plot_data,get_data, level_data, save_data, rotate_data, remove_nan_frame
...
class Data2D(object):  #np.ndarrays
    ...
    def rotate(self,angle,*args,**kwargs):
        self.data,self.x,self.y=rotate_data(self.data,self.x,self.y,angle,*args,**kwargs) 
        return self
        
    def transpose(self):
        self.data,self.x,self.y = transpose_data(self.data,self.x,self.y)
        return self
    
    def apply_transform(self,*args,**kwargs):
        self.data,self.x,self.y=apply_transform(self.data,self.x,self.y,*args,**kwargs)
        return self
    
    def crop(self,*args,**kwargs):
        self.data,self.x,self.y=crop_data(self.data,self.x,self.y,*args,**kwargs) 
        return self

    def level(self,*args,**kwargs):
        self.data,self.x,self.y=level_data(self.data,self.x,self.y,*args,**kwargs) 
        return self
    ...
    def remove_nan_frame(self,*args,**kwargs):
        self.data,self.x,self.y=remove_nan_frame(self.data,self.x,self.y,*args,**kwargs)  
        return self       
