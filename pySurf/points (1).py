...
def crop_points2(points,xrange=None,yrange=None,zrange=None,mask=None,boolean=False):
    ...
    result=outmask if boolean else points[outmask]
...
def points_find_hull(pts):
    ...
    cv = ConvexHull(pts[~np.isnan(pts[:,2]),:2])

    ihull = cv.vertices
    # the vertices of the convex hull
    hull=pts[~np.isnan(pts[:,2]),:2][ihull,:]
...
##      in data2D. The argument cut was also introduced in resample_grid to overmcome this problem 
...
def plot_points(points,xgrid=None,ygrid=None,shape=None,resample=True,scatter=False,contours=0,bar=True,
    **kwargs):
