...
def matrixZygo_reader(wfile,*args,**kwargs):
    ...
    from pySurf._instrument_reader import csvZygo_reader,read_data
...
def matrix4D_reader(wfile,*args,**kwargs):
    ...
    """
    ypix=kwargs.pop('ypix',None)
    ytox=kwargs.pop('ytox',1.)
    zscale=kwargs.pop('zscale',1.)
    scale=kwargs.pop('scale',1.)
    
    data,x,y=csv4D_reader(wfile,*args,**kwargs)
    return register_data(data,x,y,*args,**kwargs)
    """
...
def matrixsur_reader(wfile,*args,**kwargs):
    ...
    from _instrument_reader import sur_reader,read_data
    return read_data(wfile,sur_reader,*args,**kwargs)
    """
    data,x,y=sur_reader(wfile,*args,**kwargs)
    return register_data(wdata,x,y,*args,**kwargs)
    """
...
def fits_reader(wfile,*args,**kwargs):
    ...
    from pySurf._instrument_reader import fits_reader,read_data
...
def fitsWFS_reader(wfile,*args,**kwargs):
    """temporary wrapper for new readers, replace call to fitsWFS_reader
    with calls to read_data(wfile,reader=fits_reader)"""
    
    wdata,x,y=fits_reader(wfile,*args,**kwargs)
    return register_data(wdata,x,y,*args,**kwargs)
'''  
